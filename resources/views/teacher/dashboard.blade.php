    ...
    <div class="p-4 sm:p-6 lg:p-8 space-x-4 sm:space-x-6">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            {{-- <a href="{{ route('trackers.create') }}"
                class="inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition duration-150 ease-in-out text-sm sm:text-base">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mt-4">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 sm:p-6">
                <div class="text-center">
                    <div class="text-2xl sm:tex-3xl font-bold text-yellow-600 dark:text-yellow-400">{{ $stats['pending']
                        }}</div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 sm:p-6">
                <div class="text-center">
                    <div class="text-2xl sm:tex-3xl font-bold text-green-600 dark:text-green-400">{{ $stats['reviewed']
                        }}</div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 sm:p-6">
                <div class="text-center">
                    <div class="text-2xl sm:tex-3xl font-bold text-red-600 dark:text-red-400">{{ $stats['rejected'] }}
                    </div>
                    <div class="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 mt-1 sm:mt-2">{{ __('Rejected') }}
                    </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border mt-4 border-zinc-200 dark:border-zinc-700 rounded-lg p-4 sm:p-6">
            <h2 class="text-lg sm:text-xl font-semibold text-zinc-900 dark:text-zink-100 mb-3 sm:mb-4">{{ __('Recent
                Submissions') }}</h2>
            ...
            <div class="block lg:hidden space-y-3">
                ...
                <div class="border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-2">
                    <div class="flex justify-between items-start">
                        <div class="flex-1 min-w-0">
                            <h3 class="font-medium text-zinc-900 dark:text-zinc-100 text-sm">{{
                                $tracker->trackerTypes->name }}</h3>
                            <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-1">
                                {{ $tracker->period_start->toFormattedDateString() }} - {{
                                $tracker->period_end->toFormattedDateString() }}
                            </p>
                    ...
                    <div class="text-xs text-zinc-500 dark:text-zinc-400">
                        {{ __('Submitted') }} {{ $tracker->submission_date->toFormattedDateString() }}
                ...
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                        ...
                        <tr>
                            ...
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-600 dark:text-zinc-400">
                                {{ $tracker->period_start->toFormattedDateString() }} - {{ $tracker->period_end->toFormattedDateString()}}
                            </td>
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-600 dark:text-zinc-400">
                                {{ $tracker->submission_date->toFormattedDateString() }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <a href="{{ route('trackers.show', $tracker) }}"
                                    class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:text-blue-300">{{
                                    __('View') }}</a>
    ...
    <flux:modal name="create-tracker" class="md:w-[600px] space-y-6">
        ...
        <form action="{{ route('trackers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <flux:field>
                ...
                <input type="file" name="file" id="file" accept=".pdf, .doc, .docx" class="block w-full text-sm text-zinc-500 dark:text-zinc-400
                        ...
                        dark:hover:file:bg-blue-800" required>
            ...
            <flux:field>
                ...
                <flux:textarea name="notes" rows="3" placeholder="{{ __('Add any additional notes or comments') }}">{{
